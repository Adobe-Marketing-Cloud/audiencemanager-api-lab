...
* Use curl to send a request to https://api-sandbox.demdex.com/oauth/token to get your access token and refresh token. For this exercise, you'll be using the OAuth2 password flow where with a single request you exchange client id, client secret, username, and user's password for tokens.
```bash
...
* Now we will check whether our access_token is working for AAM API access.

```bash
curl https://api-sandbox.demdex.com/v1/segments -H 'Authorization: Bearer PLACE-YOUR-TOKEN-HERE'
```

* After that, you need to place the access_token in a config file located inside the lab project. The location of the file is: `configs/aam_configs.py`
...
* Now write the following code inside the file to implement a basic API exchange:
```python
...
* Create a method for checking http response codes:
```python
def successful(response):
    return response is not None and response.status_code in [200, 201, 204]
```

* Create a method for getting self-user information:
```python
def get_self():
...
* Data Sources signify where a visitor data came from. Traits and Segments must be associated with a Data Source. We will first check whether a Shop Data Source already exists in AAM by querying the API using an Integration Code “shop”. If the Data Source does not exist, we will create one automatically.
...
* Test the method from `app/models.py`
```python
from . import aam.py
...
* Check your terminal logs again to see the output from Data Source API calls.


### Exercise-04: Create Trait folders for each Category automatically
* Traits are stored in folder taxonomies. We will create a Trait folder for each product category automatically.
...
* Find and add these two new lines in `app/models.py` for triggering the methods we just created automatically when we create or update a Category in our sample app:
```python
...
* Now go to Shop Administration in your browser: http://localhost:8000/admin

* And try adding a new Category.

* Open Adobe Audience Manager (Sandbox) in a separate browser tab by visiting: https://bank-sandbox.demdex.com and using your AAM lab credentials.

* If you navigate to the Traits tab, you will notice that a new folder was automatically created with the same name as your Product Category.

* Now go back to Shop Administration and update one of the existing Categories.
...
* You can also try renaming an existing Category and see the name change being reflected in AAM.


### Exercise-05: Create Traits for each Product automatically
* We will auto create a trait to represent a visitor who is interested in a product. To do this, we will implement few methods to GET, POST and PUT traits on AAM side.
...
* We will add these methods in `app/models.py` for triggering them automatically when a product is added or modified:
